package com.human.ex20221019;

import java.util.ArrayList;

public class Prac06 {
	public static void main(String[] args) {
		SmartPhone myPhone = new SmartPhone("010-1234-5678", "갤럭시Z플립", "검은색", "김민기", "5G");
...
//		myPhone.search("주차장");
		...
		myPhone.appList.add(app1);
		myPhone.appList.add(app2);
		myPhone.appList.add(app3);
		
		Phone oldPhone = new Phone("010-1234-5677", "햅틱", "검은색", "김민수");
		
		Callable ca = oldPhone;
//		ca.call("119");
		
		//message2 외부에서 이ㅣㅁ 어떤 앱을 통해서 메세제ㅣ를 보낼지 결정이 되었을때
		
		myPhone.message("라인", "이건 안터진대1");
		myPhone.message("카카오톡", "이건 안터진대2");
		myPhone.message("인스타", "이건 안터진대3");
		//찾는과정
		//app2 //라인
		myPhone.message2(app2, "빨리 퇴근해1");
		myPhone.message2(app1, "빨리 퇴근해2");
		myPhone.message2(app3, "빨리 퇴근해3");
...
//		sm.sendMsg(text);
	...
	//가지고 있는 채팅앱이 3개. message -> 실제 메세지 전송 -> 
	//SendMessage 인터페이스 구현한 sendMsg() 메소드를 가진 객체
...
class ChattingApp extends App implements SendMessage{
	...
	public void sendMsg(String text) {
		// TODO Auto-generated method stub
		System.out.println(text + " 메세지를 보냅니다.");
	}
}

interface SendMessage{ //메세지 보내는 기능
	public void sendMsg(String text); //메세지 보내기
}