...
//	PayHelp payMachine = new PayHelpA();
	PayHelp payMachine;
	...
	public void pay() { //메소드는 최소의 기능
		//가격을 체크하는 부분 -> 메뉴리스트에서 고객이 선택한(담아놓은) 상품들의 가격을 총합
		
		payMachine.payCard(1000);
		
		payMachine.payCash(2000);
		
		payMachine.payMobile(3000, 20);
...
//	public boolean payMobileToss(int amount);	
...
class PayHelpB implements PayHelp{
	...
	public boolean payMobile(int amount, int option) {
		...
		if(option == PayHelp.MOBILE_KAKAO) {
			...
			System.out.println("네이버 페이를 지원하지 않습니다.");
			result = false;
		}else if(option == PayHelp.MOBILE_ZERO) {
			//우리는 제로 페이를 지원하지 않습니다.
			System.out.println("제로 페이를 지원하지 않습니다.");
			result = false;
		}
		return result;
	}
